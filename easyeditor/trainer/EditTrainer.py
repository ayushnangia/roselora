...
class EditTrainer(BaseTrainer):
    ...
    def edit_step(self, batch, training: bool):
        ...
        with torch.set_grad_enabled(training):
            ...
            if 'llama' in self.config.model_class.lower():
                NULL_TOKEN = 0

                pred = post_edit_logits
                ...
                # smooth label 
                smooth_labels = torch.zeros_like(pred).to(pred)
                smooth_labels.scatter_(2, targ.unsqueeze(-1), 1.0)

                smoothing = 0.99
                if self.prev_model is not None and self.do_label_smoothing:
                    with torch.no_grad():
                        prev_logits = self.prev_model(**batch["edit_inner"])
                        prev_pred = prev_logits[..., :-1, :]
                        prev_pred = prev_pred[:, -targ.size(1):]
                        prev_pred = torch.softmax(prev_pred, dim=-1).detach()
                        smooth_labels = smoothing * smooth_labels + (1 - smoothing) * prev_pred
                        
                        max_smooth_labels = smooth_labels.argmax(dim=-1)
                        correct_max = (max_smooth_labels == targ)[mask]
                        correct_smoothing = correct_max.all().item()
                        assert correct_smoothing 

                log_probs = torch.log_softmax(pred, dim=-1)
                sample_loss = -(smooth_labels * log_probs).sum(dim=-1)
                masked_loss = sample_loss * mask
                l_edit = masked_loss.sum() / mask.sum()
    ...
    def train_step(self, batch):
        ...
        if self.global_iter > 0 and self.global_iter % self.config.accumulate_bs == 0:
            ...
            if self.do_label_smoothing:
                if self.prev_updates == 100 or self.prev_model is None:
                    with torch.no_grad():
                        if "cond" in batch:
                            edited_model, _ = self.model.edit(batch["edit_inner"], batch["cond"])
                        else:
                            edited_model, _ = self.model.edit(batch["edit_inner"])
                        self.prev_model = edited_model
                    self.prev_updates = 1
