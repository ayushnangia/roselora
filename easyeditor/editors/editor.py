...
class BaseEditor:
    ...
    def load_model(self, hparams):
        ...
        if type(self.model_name) is str:
            ...
            torch_dtype = dtype_mapping[hparams.dtype]
            ...
            # Ours
                ...
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name, torch_dtype=torch_dtype, device_map=device_map, token=token, trust_remote_code=True)
                self.tok = AutoTokenizer.from_pretrained(self.model_name, token=token, model_max_length=256, use_fast=False, padding_side="left", trust_remote_code=True)
                self.tok.pad_token_id = self.tok.eos_token_id
            elif 'gemma-1.1' in self.model_name.lower():
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name, torch_dtype=torch_dtype, device_map=device_map, token=token, trust_remote_code=True)
                self.tok = AutoTokenizer.from_pretrained(self.model_name, token=token, model_max_length=256, use_fast=False, padding_side="left", trust_remote_code=True)
                self.tok.pad_token_id = self.tok.eos_token_id
            else:
    ...
    def batch_edit(self,
        ...
        if ground_truth is not None:
            ...
            # Default ground truth is <|endoftext|>
        ...
        if sequential_edit and sequential_edit_size > 1:
            edited_model = self.model
            weights_copy = None
            seq_request = []
            seq_ids = []
            seq_chunks = []
            batches = math.ceil(len(requests) / self.hparams.batch_size)

            for chunk_i, record_chunks in enumerate(tqdm(_chunks(requests, self.hparams.batch_size), total=batches)):
                edited_model, weights_copy_, icl_examples = edit_func(self.model, record_chunks)

                # Avoid weights_copy overwriting the real original states. 
                if weights_copy is None:
                    weights_copy = weights_copy_
                
                seq_request += record_chunks

                # The last batch must be complete. Otherwise the algorithm will be ended
                seq_ids += list(range(chunk_i * self.hparams.batch_size, chunk_i * self.hparams.batch_size + len(record_chunks)))
                seq_chunks.append(chunk_i)

                # Evaluate the running batch
                # We only care how many sequential edits happen, but not how many batches it contains
                if len(seq_chunks) >= sequential_edit_size or chunk_i == batches - 1:
                    for _id, request in enumerate(seq_request):
                        edit_evaluation(all_metrics, request, edited_model, seq_ids[_id], test_generation, icl_examples, **kwargs)

                    print(f"Finish {sequential_edit_size} Sequential Edits...", "\n")
                    ...
                    print(f"Reset Criteria and model...", "\n")
            ...
            for record_chunks in _chunks(requests, self.hparams.batch_size):
                ...
                edited_model, weights_copy, icl_examples = edit_func(self.model, record_chunks)

                exec_time = time() - start
                LOG.info(f"Execution editing took {exec_time}")

                for i, request in enumerate(record_chunks):
                    edit_evaluation(all_metrics, request, edited_model, run_size+i, test_generation, icl_examples, **kwargs)
                run_size += len(record_chunks)
    ...
    def edit(self,
        ...
        return self.edit_requests(requests, sequential_edit, sequential_edit_size, verbose, test_generation=test_generation, **kwargs)
    ...
    def edit_requests(self,
        ...
        def edit_evaluation(all_metrics, request, edited_model, idx, test_generation, icl_examples, **kwargs):
            ...
            if self.alg_name == 'IKE':
                ...
                all_metrics[idx].update({
                    'case_id': idx,
                    "requested_rewrite": request,
                    "post": compute_edit_quality(edited_model, self.model_name, self.hparams, self.tok, request, self.hparams.device, eval_metric=eval_metric, test_generation=test_generation, is_qa=is_qa, idx=idx),
                })
        ...
        if 'pre_edit' in kwargs and kwargs['pre_edit'] is not None:
            ...
            for i, request in enumerate(tqdm(requests)):
                if self.alg_name == 'IKE':
                    assert 'train_ds' in kwargs.keys(), print('IKE need train_ds(For getting In-Context prompt)')
                    metrics = {
                        "pre": compute_icl_edit_quality(self.model, self.model_name, self.hparams, self.tok, [''], request, self.hparams.device, pre_edit=True)}
                else:
                    metrics = {"pre": compute_edit_quality(self.model, self.model_name, self.hparams, self.tok, request, self.hparams.device, eval_metric=eval_metric, test_generation=test_generation,  is_qa=False, idx=i)}
                all_metrics.append(metrics)
            if 'pre_file' in kwargs and kwargs['pre_file'] is not None:
                json.dump(all_metrics, open(kwargs['pre_file'], 'w'), indent=4)
        ...
        if sequential_edit and sequential_edit_size > 1:
            ...
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                if self.alg_name in ["LoRA", "RoseLoRA"]:
                    edited_model, weights_copy_, icl_examples = edit_func(edited_model, request)
                else:
                    edited_model, weights_copy_, icl_examples = edit_func(self.model, request)
                # Avoid incorrect reset
                if weights_copy is None:
                    weights_copy = weights_copy_

                seq_request.append(request)
                seq_ids.append(i)

                # Evaluate the running batch
                if len(seq_request) >= sequential_edit_size or i == len(requests) - 1:
                    ...
                    reset_seq_after_batch = kwargs.get("reset_seq", True)
                    if reset_seq_after_batch and i != len(requests) - 1:
                        if self.alg_name == 'KN' or self.alg_name == 'GRACE' or self.alg_name == 'WISE':
                            with torch.no_grad():
                                weights_copy()

                        elif self.alg_name == 'LoRA' or self.alg_name == 'QLoRA' or self.alg_name == 'DPO' or self.alg_name == 'RoseLoRA':
                            edited_model.unload()
                            del self.model.peft_config
                            edited_model = self.model
            ...
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                edited_model, weights_copy, icl_examples = edit_func(self.model, request)

                if self.alg_name == "MELO":
                    edited_model = edited_model.model
