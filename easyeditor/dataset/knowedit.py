...
class KnowEditDataset(Dataset):
    ...
    def __init__(self, data_dir: str, size: typing.Optional[int] = None, rand_subset = False, config=None, *args, **kwargs):
        ...
        if size is not None:
            if rand_subset:
                rng = random.Random(42)  # Create separate random number generator
                indices = rng.sample(range(len(data)), size)
