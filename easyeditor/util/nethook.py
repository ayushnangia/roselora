...
def get_parameter(model, name):
    """
    Finds the named parameter within the given model, handling both tied and untied weights.
    Returns the parameter if found, raises LookupError if not found.
    """
    # First try direct parameter lookup
    for n, p in model.named_parameters():
        if n == name:
            return p
        # if "lm_head" in n:
        #     print("Found param with lm_head:", n)
    
    # If looking for lm_head weight specifically
    if name == "lm_head.weight" and hasattr(model, 'lm_head'):
        # Case 1: Weight exists directly but not in named_parameters
        if hasattr(model.lm_head, 'weight'):
            return model.lm_head.weight
            
        # Case 2: Check for tied embeddings
        if hasattr(model, 'get_input_embeddings'):
            try:
                # Some models might have get_input_embeddings but not use it for tying
                weight = model.get_input_embeddings().weight
                if weight.shape == model.lm_head.out_features: # Basic shape check
                    return weight
            except (AttributeError, RuntimeError):
                pass
                
        # Case 3: Check for wrapped linear layer
        if hasattr(model.lm_head, 'linear'):
            return model.lm_head.linear.weight
            
        # # Case 4: Debug info if none of the above worked
        # print("\nDebug info for lm_head:")
        # print("LM head attributes:", dir(model.lm_head))
        # print("LM head structure:", model.lm_head)
        
    raise LookupError(f"Could not find parameter: {name}")
...
#     print(list(model.lm_head.named_parameters()))
