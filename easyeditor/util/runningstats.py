...
def tally(stat, dataset, cache=None, quiet=False, **kwargs):
    ...
    for k in ["sample_size"]:
        if k in kwargs:
            args[k] = kwargs[k]
    cached_state = load_cached_state(cache, args, quiet=quiet, throw=False)
...
def load_cached_state(cachefile, args, quiet=True, throw=True):
    """
    Resolves a state, which can be a filename or a dict-like object.
    """
    ...
    try:
        if isinstance(cachefile, dict):
            ...
            # dev = numpy.load(cachefile)
            # # print(dev)
            # # unbox_numpy_null(dev)
            # # exit(0)
