...
def compute_edit_quality(
    ...
    if not silence:
        print(f"Eval Success [ID {idx}]: {rewrite_prompts} -> {target_new}")
    ...
    # loc_gts = []
    # for locality_key in record['locality'].keys():
    #     loc_gts += record['locality'][locality_key]['ground_truth']
    # print("Real: ", loc_gts)

    # loc_toks = []
    # for loc in ret["locality"].values():
    #     loc_toks += loc
    # print("Predicted: ", tok.batch_decode(loc_toks))
    # test_generation = False

    if 'portability' in record.keys() and any(record['portability']):
        # if not silence:
            # print("Eval Portability:  ")
        for portability_key in record['portability'].keys():

            # Walk around solution: 
            # use portability to feed downstream locality text here
            if "down_" in portability_key:
                down_score = compute_rewrite_or_rephrase_quality(model, model_name, hparams, tok,
                                                        record['portability'][portability_key]['prompt'],
                                                        record['portability'][portability_key]['ground_truth'], 
                                                        device=device,
                                                        test_rephrase=True, eval_metric="ppl")
                ret['portability'].update({portability_key: list(down_score.values())[0]})
            else:
                ret['portability'].update(
                    compute_portability_quality(model, model_name, hparams, tok, portability_key,
                                                record['portability'][portability_key]['prompt'],
                                                record['portability'][portability_key]['ground_truth'], 
                                                device=device, is_qa=is_qa, use_cot="2hops" in portability_key)
                )
    if test_generation:
...
def compute_portability_quality(
    ...
    if not is_qa:
        ...
        if isinstance(prompt, str):
            prompt, ground_truth = [prompt,], [ground_truth,]
        
        portability_correct = []
        for p, g in zip(prompt, ground_truth):
            portability_correct_i = answer_match(model, tok, hparams,  p, g, device, use_cot=use_cot)
            portability_correct.append(float(portability_correct_i))
